    ...
    <section className="relative w-screen min-h-screen bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${image})` }}>
      ...
      <div className="relative z-10 flex items-end min-h-screen pl-8 md:pl-16 lg:pl-32 pb-20">
        ...
        >
          ...
          >
            {ctaText && ctaLink && (
              <Link to={ctaLink} className="bg-[#009FF3] text-white px-6 py-3 font-inter font-medium tracking-tight hover:bg-[#0080cc] transition-colors">
                {ctaText}
              </Link>
            )}
            {cta2Text && cta2Link && (
              <Link to={cta2Link} className="bg-white text-black px-6 py-3 font-inter font-medium tracking-tight hover:bg-gray-100 transition-colors">
                {cta2Text}
              </Link>
            )}
          </motion.div>
